...
const incrementState = new Map();
/** Snowflake state (module-level singleton) */
let snowflakeState = null;
// Snowflake constants
 ...
 */
export function generateId(strategy = 'uuid', config) {
    switch (strategy) {
        case 'uuid':
            return generateUuid();
        case 'snowflake':
 ...
 */
function generateSnowflake(config) {
    // Validate config
    if (config.workerId < 0 || config.workerId > Number(MAX_WORKER_ID)) {
        throw new Error(`Worker ID must be between 0 and ${Number(MAX_WORKER_ID)}, got ${config.workerId}`);
    }
    if (config.datacenterId < 0 ||
        config.datacenterId > Number(MAX_DATACENTER_ID)) {
        throw new Error(`Datacenter ID must be between 0 and ${Number(MAX_DATACENTER_ID)}, got ${config.datacenterId}`);
    }
    ...
    if (now < snowflakeState.lastTimestamp) {
        throw new Error(`Clock moved backwards. Refusing to generate ID for ${snowflakeState.lastTimestamp - now}ms`);
    }
    if (now === snowflakeState.lastTimestamp) {
    ...
    const id = (timestamp << (WORKER_ID_SHIFT + DATACENTER_ID_SHIFT + SEQUENCE_BITS)) |
 ...
 */
function generateIncrement(collection) {

...
export class PragmaManager {
   ...
   * Set a pragma value.
   */
  async setPragma(name: PragmaName, value: PragmaValue): Promise<void> {
    wrapNative(() =>
      native.setPragma(this.#handle(), name, JSON.stringify(value)),
    );
  }
   ...
   * Read a pragma value.
   */
  async getPragma(name: PragmaName): Promise<PragmaValue> {
    const json = wrapNative(() => native.getPragma(this.#handle(), name));
    return JSON.parse(json) as PragmaValue;
  }
}

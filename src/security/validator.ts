 ...
 */
export function validateDepth(value: unknown, maxDepth = DEFAULT_MAX_DEPTH, currentDepth = 0): void {
 ...
 */
export function validateSize(doc: unknown, maxSize = DEFAULT_MAX_SIZE): void {
  ...
  if (collection && config.allowedFields) {
    ...
    if (allowed) {
      ...
      if (violations.length > 0) {
        throw new Error(
          `Disallowed fields in collection '${collection}': ${violations.join(', ')}`,
        );
      }
    }
  }
